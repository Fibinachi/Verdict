...
- **2026-05-29 (v0.75 — The Damages Package)**: Jury now deliberates on and awards damages in civil cases. Deliberation prompt includes damages context (medical costs, lost wages, pain & suffering, punitive factors). `ParseDamagesFromStatement` extracts dollar amounts ($X, X million, Xk, X billion) from juror statements. Damages influence propagates among jurors during deliberation. `ConcludeDeliberation` computes median damages award from liable jurors, sets `VerdictForm.DamagesAwarded`, and updates `EstimatedSettlement` (85%) and `InsuranceReserve` (115%). Transcript and questionnaire display per-juror damages and damages consensus. `Agent.UpdateValuationFromDamages` formats as $XK/$X.XM. All tests passing, 0 failures.
